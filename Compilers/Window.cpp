...
{
  #ifdef P5
    Table_handler& table_handler = Table_handler::instance();
    std::shared_ptr<Symbol> symbol = table_handler.lookup("mouse_x");
    if (symbol != nullptr && symbol->get_type() == INT)
      symbol->as_lvalue()->mutate(x);
  
    symbol = table_handler.lookup("mouse_y");
    if (symbol != nullptr && symbol->get_type() == INT)
...
void Window::set_width(int width)
{
    ...
    std::shared_ptr<Symbol> symbol = table_handler.lookup("window_width");
    if (symbol != nullptr && symbol->get_type() == INT)
...
void Window::set_height(int height)
{
    ...
    std::shared_ptr<Symbol> symbol = table_handler.lookup("window_height");
    if (symbol != nullptr && symbol->get_type() == INT)
...
void Window::dump_pixels(const char *dumpwindow_filename)
{
  ...
  shared_ptr<char> pixels(new char[m_w * m_h * 3]);
