...
/* static */ map<string, std::shared_ptr<Pixmap::Pixmap_data>> Pixmap::m_pixmap_cache;

Pixmap::Pixmap()
  : Game_object(0.5, 0.5, 0.5, PIXMAP), m_pixmap_data(nullptr), m_tried_to_read_current_file(false)
{
  Register("filename", std::string(""));
  // if the default pixmap has yet to be created, create it
  if (!default_pixmap_data)
  {
    default_pixmap_data = make_shared<Pixmap_data>(default_width,
                                          default_height,
                                          default_data
                                         );
  }
...
void Pixmap::updated(const string& name)
{
...
void Pixmap::read_file()
{
  ...
  map<string, std::shared_ptr<Pixmap_data>>::const_iterator iter;
  ...
  {
    cerr << "Texture filename <" << attribute<std::string>("filename") << "> not found." << endl;
    return;
  }
  ...
  if ((i = fread(&data_offset, 4, 1, file)) != 1)
  {
    cerr << "Error reading data_offset size from texture filename <"
         << attribute<std::string>("filename") << ">" << endl;
    return;
  }
  ...
  if (planes != 1)
  {
    cerr << "Error reading texture filename <" << attribute<std::string>("filename")
  ...
  {
    cerr << "Error reading bits per pixel from texture filename <" << attribute<std::string>("filename") << endl;
    return;
  }
  ...
  // cout << attribute<int>("filename") << " being parsed\n";
  ...
  m_pixmap_data = make_shared<Pixmap_data>(width, height, bitmap_data_with_alpha);
  attribute<int>("w")=object_width=m_pixmap_data->m_width;
  attribute<int>("h")=object_height=m_pixmap_data->m_height;
