...
void MainWindow::numberGroup_clicked(QAbstractButton* button)
{
    ...
    //Append requested digit to final string
    //expression.append(displayLabel);
...
void MainWindow::actionGroup_clicked(QAbstractButton* button)
{
    ...
    if (operatorClicked) {
        ...
        //expression.append(storedOperator);
    }
    else {
        if (hasStoredNumber) {
            calculate_result();
...
void MainWindow::calculate_result() {
    ...
    //save the original two digets
    didget1 = QString::number(storedNumber,'g', DIGIT_LIMIT);
    didget2 = displayLabel;
     ...
     //Set number back to display
     //prints out full expression ex. 4 + 4 = 8
     ui->displayPanel->setText(didget1 + " " + storedOperator + " " + didget2 +" = "+  displayLabel);
     didget1 = QString::number(storedNumber,'g', DIGIT_LIMIT);

    //to just print answer delete above 2 lines and uncomment line below
     //ui->displayPanel->setText(displayLabel);
